...
class Profile(models.Model):
    ...
    profile_reference = models.IntegerField(auto_created=True,
            primary_key=True, unique=True)
    user_id = models.ForeignKey(User, to_field='username',
    ...
    email = 'email'
    post = 'post'
    preference_of_contact = models.CharField(max_length=6,
            choices=((phone, 'Phone'), (post, 'Post'), (post, 'letter'
            )))
...
class Booking(models.Model):
    ...
    booking_reference = models.IntegerField(unique=True,
            primary_key=True, null=False)
    booker = models.ForeignKey(User, to_field='username',
    ...
    provided_number = models.CharField(max_length=11, blank=True,
            null=True)
...
class OverallComment(models.Model):
    ...
    booking_reference = models.OneToOneField(Booking,
            to_field='booking_reference', on_delete=models.CASCADE)
    comment = models.CharField(max_length=250)
    happy = 'happy'
    indifferent = 'indifferent'
    unhappy = 'unhappy'
    overall_feeling = models.CharField(max_length=11, choices=((happy,
            'happy'), (indifferent, 'indifferent'), (unhappy, 'unhappy'
            )))
    ...
    def __str__(self):
        return 'comment' + str(self.o_comment_id) + 'by' \
            + str(self.booking_reference.booker)


class PersonalComment(models.Model):

    p_comment_id = models.AutoField(primary_key=True, unique=True)
    booking_reference = models.ForeignKey(Booking,
            to_field='booking_reference', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return 'comment' + str(self.p_comment_id) + 'by' \
            + str(self.booking_reference.booker)